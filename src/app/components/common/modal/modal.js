...
const CustomModal = ({ isOpen, onRequestClose, title, children }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center">
      ...
      <div
        ref={modalRef}
        className="relative bg-white rounded-2xl max-w-[700px] w-full mx-4 z-10 max-h-[80vh] flex flex-col"
        tabIndex={-1} // Make it focusable
        role="dialog"
        aria-label={title}
      >
        {/* Header */}
        <div className="p-4 px-5 border-gray-300 flex justify-between items-center shrink-0">
        ...
        {/* Scrollable Body */}
        <div className="p-3 px-4 overflow-y-auto">{children}</div>
      </div>
    </div>
  );
};
